...
fvToEPV <- function(tdat, tdat.covars, fv, ev.out) {
  ...
  # expected value from each player/location/defence pair
  evs <- ev.out$evs
  for(i in 1:nrow(ev.out$teammates.all)){
    these <- ev.out$lineup.inds[[i]]
    adder <- ev.out$adder.mat[i, ]
    #with this line up, recipient (1) state {area} x {defended}
  ...
  #epass1 <- epass1[!is.na(epass1)]
  #epass2 <- epass2[!is.na(epass2)]
  #epass3 <- epass3[!is.na(epass3)]
  #epass4 <- epass4[!is.na(epass4)]
  #enow <- epass4[!is.na(enow)]
...
multiresDraw <- function(tdat, hyper, micro.def.mod, ev.out, nmic=50, save.positions=FALSE) {
  ...
  # get hazards
    #theta covariates for fixed and spatial effects for Macrotransition
  tdat.covars <- tdatCovars(tdat)
    #hazard for each macro type fo each player
  fv <- fvMatToDF(fitVals(hyper, tdat, tdat.covars))
    #instant passing prob and expected
  ...
  for(i in 2:nmic) {
    ...
    # get hazards from updated positions
...
compressEPV <- function(tdat, fv.epv.list) {
  ...
  # integrate over time to obtain marginal transition probabilities and values
     #50 iterations of microtransition
